...
function useMissionFlowLayout() {
  const [layout, setLayout] = useState<"desktop" | "mobile" | null>(null);

  useEffect(() => {
    const desktop = window.matchMedia("(min-width: 1024px)");
    const update = () => setLayout(desktop.matches ? "desktop" : "mobile");
    update();
...
export default function MissionFlowPulse() {
  ...
  const layout = useMissionFlowLayout();

  if (layout === null) {
    return <div className="mission-flow" aria-hidden />;
  }
