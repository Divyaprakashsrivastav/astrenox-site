...
function JourneyMilestone({
  ...
  const Icon = STEP_ICONS[index % STEP_ICONS.length] ?? Compass;
  const number = String(index + 1).padStart(2, "0");
  const isCompleted = progress >= (index + 1) / total;
  const isCurrent = progress >= index / total && progress < (index + 1) / total;
  ...
  return (
    ...
    >
      <div className="adv-journey-milestone-spine" aria-hidden>
        ...
        >
          <motion.span className="adv-journey-node-num" style={{ opacity: inView ? 1 : 0.35 }}>
            {number}
          </motion.span>
          <motion.span
...
function StrategyJourneyTimeline({
  id,
  label,
  title,
  intro,
  steps,
  titleId = "strategy-journey-title",
}: StrategyJourneyTimelineProps) {
  ...
  return (
    ...
    >
      <div className="adv-journey-bg" aria-hidden>
        ...
        <svg
          className="adv-journey-bg-network"
          viewBox="0 0 800 600"
        ...
        >
          ...
          <motion.path
            ...
            transition={{
              ...
              opacity: { duration: 5, repeat: Infinity, delay: 0.5 },
            }}
          />
        </svg>
...
export default memo(StrategyJourneyTimeline);
