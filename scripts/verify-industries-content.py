...
CONTENT = Path(__file__).resolve().parents[1] / "app" / "content" / "industries-page-content.ts"
...
def main():
    ...
    for para in doc:
        if para in raw or para in flat:
            continue
        # custom items: title: body combined
        if ": " in para:
            title, _, body = para.partition(": ")
            combined = f"{title}: {body}"
            if combined in flat or title in flat and body in flat:
                continue
        missing.append(para)
